...
(function(e){AJS.asBooleanOrString=function(i){var h=i?i.toLowerCase():"";if(h=="true"){return true}if(h=="false"){return false}return i};var d=false;AJS.debug=function(h){if(!d){return}AJS.log("DEBUG: "+new Date().toLocaleTimeString()+" : "+h)};AJS.debugEnabled=function(h){if(typeof h!="boolean"){return d}d=h;AJS.log("DEBUG ENABLED: "+d)};var g={};AJS.Meta=e.extend({},AJS.Meta,{set:function(h,i){g[h]=i},get:function(i){if(typeof g[i]!="undefined"){return g[i]}var h=e("meta[name='ajs-"+i+"']");if(!h.length){return undefined}var j=h.attr("content");return AJS.asBooleanOrString(j)},getBoolean:function(h){return this.get(h)===true},getNumber:function(h){return +this.get(h)},getAllAsMap:function(){var h={};e("meta[name^=ajs-]").each(function(){h[this.name.substring(4)]=this.content});return e.extend(h,g)}});AJS.logError=function(j,h){var i=[];for(prop in h){i.push(prop+": "+h[prop])}AJS.log(j+i.join(", "))};AJS.toInit=function(h){e(function(){try{h.apply(this,arguments)}catch(i){AJS.logError("Failed to run init function: ",i)}});return this};if(AJS.Meta.getBoolean("log-rendered")){var f=AJS.log,c=e('<div id="ajs-log" class="log"><h3>AJS Log</h3>\n</div>'),b=e("head"),a;c.toggleClass("hidden",!AJS.Meta.getBoolean("log-visible"));AJS.log=function(i){var j=(typeof(i)==="undefined")?"undefined":i;if(a){if(c.next().length!=0){a.append(c)}}else{var h=document.createElement("script");h.type="text/x-log";h.text=j;b.append(h)}c.append(e("<p></p>").text("\n"+j));f(i)};AJS.toInit(function(){a=e("body");a.append(c)})}AJS.Data=AJS.Data||AJS.Meta;AJS.getJSONWrap=function(j){var i=Confluence.getContextPath();var h=j.url;if(h.indexOf(i)!=0&&h.indexOf("http")!=0){h=i+h}j.loadingElement&&AJS.setVisible(j.loadingElement,true);var k=j.messageHandler;k.clearMessages();e.ajax({type:"GET",url:h,dataType:"json",data:j.data,error:function(){j.loadingElement&&AJS.setVisible(j.loadingElement,false);k.displayMessages(j.errorMessage||"There was an error communicating with the server, please try again later.");j.errorCallback&&j.errorCallback()},success:function(l){j.loadingElement&&AJS.setVisible(j.loadingElement,false);if(k.handleResponseErrors(l)){j.errorCallback&&j.errorCallback();return}j.successCallback&&j.successCallback(l)}})};AJS.Validate=e.extend((function(){var i,h;return{email:function(j){if(!i){i=/^((([a-z]|\d|[!#\$%&amp;&#39;\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&amp;&#39;\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i}return i.test(j)},url:function(j){if(!h){h=/^([a-z]([a-z]|\d|\+|-|\.)*):(\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?((\[(|(v[\da-f]{1,}\.(([a-z]|\d|-|\.|_|~)|[!\$&'\(\)\*\+,;=]|:)+))\])|((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=])*)(:\d*)?)(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*|(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)|((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)|((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)){0})(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i}return h.test(j)}}}()),AJS.Validate)})(AJS.$);
(function(d){var a=AJS.Meta.get("enabled-dark-features"),c=a?a.split(","):[],b={};d.each(c,function(){b[this]=true});AJS.DarkFeatures={isEnabled:function(e){return !!b[e]},enable:function(e){if(e&&!b[e]){b[e]=true}},disable:function(e){if(e&&b[e]){delete b[e]}}}})(AJS.$);
...
Confluence.storageManager=function(c){var f=AJS.Meta.get("remote-user"),d="confluence."+(f?f+".":"")+c,b="#",h=false,i=/\d+#/,a=function(k){var e=(k||0)*1000;if(!e){return""}return +new Date()+e+b},j=function(k){if(!h){return null}var e;var l=localStorage.getItem(d+"."+k);if(e=i.exec(l)){l=l.replace(e[0],"");if(+new Date()>e[0].replace("#","")){localStorage.removeItem(d+"."+k);return null}}return l};try{h="localStorage" in window&&window.localStorage!==null}catch(g){AJS.log("Browser does not support localStorage, Confluence.storageManager will not work.")}return{getItem:j,doesContain:function(e){return !!j(e)},getItemAsBoolean:function(e){var k=j(e);if(k=="false"){return false}if(k=="true"){return true}return !!k},setItem:function(k,l,e){if(!h){return}l=a(e)+l;localStorage.setItem(d+"."+k,l)},removeItem:function(e){if(!h){return}localStorage.removeItem(d+"."+e)}}};
...
Confluence.Templates.PagePermissions.helpLink = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  Confluence.Templates.Dialog.helpLink({href: "http://docs.atlassian.com/confluence/docs-40/Page+Restrictions"}, output);
  if (!opt_sb) return output.toString();
};
...
AJS.toInit(function(d){var f=d("#version-comment");if(f.length){var a=d("#show-version-comment");var e=d("#hide-version-comment");a.click(function(g){a.hide();e.show();f.show();return AJS.stopEvent(g)});e.click(function(g){e.hide();a.show();f.hide();return AJS.stopEvent(g)});if(a.length&&e.length){f.hide()}}if(d("#content.edit").length&&d("#page-diffs").length){var b=d("#savebar-container").height()+d("#header").height();var c=d(window).height()-b;d("#content").height(c)}});
...
Confluence.Templates.ManageWatchers.helpLink = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  Confluence.Templates.Dialog.helpLink({href: "http://docs.atlassian.com/confluence/docs-40/Managing+Watchers"}, output);
  if (!opt_sb) return output.toString();
};
...
Confluence.Templates.MovePage.helpLink = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  Confluence.Templates.Dialog.helpLink({href: "http://docs.atlassian.com/confluence/docs-40/Moving+a+Page"}, output);
  if (!opt_sb) return output.toString();
};
...
AJS.toInit(function(d){var b=d("#breadcrumbs"),a=b.width(),j=d("li:visible",b),h=[],f=0,g=0;j.each(function(){h.push(Math.min(f+g,a));f+=g;g=d(this).outerWidth(true)});f+=g;if(f>a){AJS.debug("Add ellipsis to crumbs");for(var e=j.length-1;e>=0;e--){var l=d(j.get(e)),k=a-h[e],c=d("a",l);if(!c.length){continue}c.attr("title",c.text());c.shortenUntil(function(){return l.outerWidth(true)<=k})}}d("#ellipsis").click(function(){try{d(".hidden-crumb",b).removeClass("hidden-crumb");d(this).addClass("hidden-crumb")}catch(i){AJS.log(i)}})});
...
AJS.toInit(function(g){function d(i){return g.trim(i).length==0}var c,f=140;function e(){var k="idontthinksohal";var i=new AJS.Dialog(650,k,"update-user-status-dialog");var j=i.popup.element;i.addHeader("What are you working on?");i.addPanel("Set Status","<form class='aui update-status'><fieldset><legend class='assistive'>Status Update</legend><label for='status-text' class='assistive'>"+"Enter your status (140 character limit)"+"</label><textarea name='status-text' id='status-text'></textarea><span id='update-status-chars-left'>"+f+"</span></fieldset><div id='dialog-current-status' class='current-user-latest-status'>"+("Last update:"||"Latest:")+" <span class='status-text'></span></div></form>");i.addButton("Update",a,"status-update-button");i.addCancel("Cancel"||"Cancel",function(l){l.hide();return false});j.find(".dialog-button-panel").append("<span class='error-message'></span>");i.setError=function(l){g("#update-user-status-dialog .error-message").html(l)};if(Confluence.KeyboardShortcuts&&Confluence.KeyboardShortcuts.enabled){i.addHelpText(AJS.format("Shortcut tip: Pressing \u003cb>{0}\u003c\/b> also opens this dialog box","s"))}return i}function b(i){g(".current-user-latest-status .status-text").html(i.text);g(".current-user-latest-status a[id^=view]").each(function(){var k=g(this),j=k.attr("href");k.attr("href",j.replace(/\d+$/,i.id)).text(i.friendlyDate).attr("title",new Date(i.date).toLocaleString())})}function h(){g.getJSON(Confluence.getContextPath()+"/status/current.action",function(i){if(i.errorMessage!=null){c.setError(i.errorMessage)}else{b(i)}})}var a=function(){var i=g("#update-user-status-dialog #status-text").attr("disabled","disabled").attr("readonly","readonly").blur();var j=i.val();g(".status-update-button").attr("disabled","disabled");if(j.length>f||d(j)){return false}AJS.safe.ajax({url:Confluence.getContextPath()+"/status/update.action",type:"POST",dataType:"json",data:{text:j},success:function(k){if(k.errorMessage!=null){c.setError(k.errorMessage)}else{b(k);i.val("");setTimeout(function(){c.hide()},1000)}},error:function(m,l,k){AJS.log("Error updating status: "+l);AJS.log(k);c.setError("There was an error - "+k)}})};g("#set-user-status-link").click(function(k){var j=g(this).parents(".ajs-drop-down")[0];j&&j.hide();if(typeof c=="undefined"){c=e();var l=g("#update-status-chars-left");var i=g(".status-update-button").attr("disabled","disabled");g("#update-user-status-dialog form.update-status #status-text").keydown(function(m){if(m.which==13){a()}}).bind("blur focus change "+(g.browser.mozilla?"paste input":"keyup"),function(){var m=f-g(this).val().length;l.removeClass("over-limit").removeClass("close-to-limit").text(m);i.removeAttr("disabled");if(d(g(this).val())){i.attr("disabled","disabled")}if(m<0){l.addClass("over-limit").html("&minus;"+-m);i.attr("disabled","disabled")}else{if(m<20){l.addClass("close-to-limit")}}});g("#update-user-status-dialog form.update-status").submit(function(m){a();return AJS.stopEvent(m)})}c.setError("");h();c.show();g("#update-user-status-dialog #status-text").removeAttr("readonly").removeAttr("disabled").focus();return AJS.stopEvent(k)})});
...
AJS.bind("register-contexts.keyboardshortcuts", function(e, data){
    ...
    // which keyboard shortcut contexts are applicable to a page.
        ...
        $("#wiki-editor").length && shortcutRegistry.enableContext("editor");
...
Confluence.KeyboardShortcuts.getDialog = function () {
    ...
    makeEditorShortcutsMenu = function() {
        var edShortcuts = Confluence.KeyboardShortcuts.Editor;
        ...
        editorShortcutsMenu.append(makeShortcutModule("Block Formatting", ["editor.block"], edShortcuts));
        editorShortcutsMenu.append(makeShortcutModule("Rich Formatting", ["editor.rich"], edShortcuts));
        editorShortcutsMenu.append(makeShortcutModule("Editing Actions", ["editor.actions"], edShortcuts));
...
Confluence.Templates.KeyboardShortcutsDialog.keyboardShortcutHelpLink = function(opt_data, opt_sb) {
  ...
  Confluence.Templates.Dialog.customHelpLink({href: soy.$$escapeHtml("http://docs.atlassian.com/confluence/docs-40/Keyboard+Shortcuts"), text: soy.$$escapeHtml("View All Shortcuts")}, output);
  output.append('</div>');
  if (!opt_sb) return output.toString();
};
